...
2. **启动时错误弹出权限请求**
   ...
   - 影响：用户体验混乱，不知道应该做什么
   - 当前处理：启动只做静默检查；全局热键监听仅在辅助功能已授权时启动；缺权限时自动打开设置窗口

3. **权限检测不准确**
   - 症状：系统设置中已授权，应用仍显示"未授权"
   - 已定位：辅助功能需要稳定签名和正确跳转；麦克风在 Hardened Runtime 下还必须具备 `com.apple.security.device.audio-input` entitlement
...
- Personal terms are provider-independent and only process final text. Users store the intended spelling; a cache for the current small term list dynamically derives Chinese, English/CamelCase, letter-name, digit, symbol, and mixed-script pronunciation signatures. Weighted pronunciation retrieval handles arbitrary user terms without built-in term-specific aliases. CMUdict and pinyin-data are general pronunciation resources, not a built-in list of product terms. High-confidence formatting-sensitive or anchored/long exact-homophone terms honor the configured spelling directly; short ambiguous Chinese homophones use the local int8 multilingual masked-language model for context. Legacy exact replacements migrate into terms with explicit aliases.
- The personal-term semantic model is `onnx-community/distilbert-base-multilingual-cased-ONNX` revision `2e7303d946cfc9194a939e02efb46824eb440379`, file `onnx/model_int8.onnx` (SHA-256 `4fa42d6f6e7d00dd734cdff3fd55b446dec3439de3f8c2e1d162e056969be343`). It is loaded lazily only for ambiguous candidates that require context scoring.
- Pinyin retrieval uses `mozillazg/pinyin-data` revision `923b108dc5d45dee061324c011b478fb649f8b73`. English retrieval uses `cmusphinx/cmudict` revision `74790861f652b15e4ac49015a90074ad62a27690` (`cmudict.dict` SHA-256 `81917843c7f44ce2b094ac63873c2c7a4cf802040792c455ba3ca406891c3d22`, license SHA-256 `bd4ce8e44170a5f9f481310ca85c51de3c4f851a65e679b40e603b143bd3542a`). English ARPABET and Chinese pinyin are mapped into the same phoneme space; unknown English spellings use a lightweight G2P fallback. All pronunciation resources and licenses are checksum-verified and bundled by `install-local-asr-models.sh`.
- Local streaming Paraformer (sherpa-onnx online), Qwen realtime, and Doubao realtime integrations have all been implemented.
...
- GuGuTalk requires only Microphone and Accessibility. Global hotkeys use the same Accessibility grant as active event handling and text insertion; do not add Input Monitoring as a separate required permission.
- Global hotkeys must use one active `CGEvent` tap as their only physical-key source. Do not add a timer that polls `CGEventSource.flagsState` for Globe/Fn: real hardware can expose only a short-lived function flag while the key remains held, causing a false release. After a fresh grant, macOS 26 can expose Accessibility, ListenEvent, and PostEvent readiness at different times in the old process; an event tap that merely exists is not proof that it can receive physical Fn events. Hotkey startup therefore requires `AXIsProcessTrusted()`, `CGPreflightListenEventAccess()`, and `CGPreflightPostEventAccess()`, then treats successful event-tap activation as the final readiness check. Any failure triggers at most one fresh-process relaunch, and the attempt marker clears only after the tap starts or Accessibility is revoked. When idle, refresh/wake/unlock rebuilds the tap instead of trusting stale validity flags. The verified production path prefers a single HID tap and falls back to a single session tap if HID creation fails; do not run both simultaneously.
- Accessibility permission requests must be one-shot per app process. The first action triggers only the macOS system prompt; do not also open System Settings in that same action. After a prompt has been requested, later actions should navigate directly to the relevant settings pane instead of requesting another prompt.
...
- Each `AVAudioConverter` input callback must supply a captured microphone buffer only once. The converter may request input multiple times per conversion; returning the same buffer repeatedly duplicates audio and can make streaming ASR emit repeated characters. `OneShotAudioConverterInput` returns `.noDataNow` after its first delivery.
- The historical Doubao repetition repair does not cover local Paraformer. Local partial/final raw text must pass through `LocalRepetitionNormalizer` before punctuation; its `NLTokenizer`-based rules remove only narrow cross-token repeats (`时候 + 候`, `整 + 整个`, `有 + 有`) while preserving normal in-token reduplication and legitimate multi-character word boundaries. Do not replace it with unconditional adjacent-character dedupe.
- Local mode uses the shared `16k Int16` PCM from `AudioCaptureEngine` and decodes each ordered chunk with streaming Paraformer. It emits punctuated partial updates while recording, then flushes and emits one punctuated final on release.
- macOS local mode no longer requires Apple Speech, Speech Recognition privacy permission, system Dictation, or Input Monitoring. It requires Microphone and Accessibility.
- sherpa-onnx runtime files live under ignored `macos/ThirdParty/sherpa-onnx/`; the model installer is `macos/scripts/install-local-asr-models.sh`. Xcode bundles streaming Paraformer (`encoder.int8.onnx`, `decoder.int8.onnx`, `tokens.txt`) plus CT-Transformer punctuation (`model.int8.onnx`) into `Contents/Resources/models/`. The verified model payload is about 298 MiB; each file is pinned by SHA-256.
- The macOS deployment target is 15.5 because bundled ONNX Runtime 1.24.4 declares 15.5 as its minimum. Lowering the app target without replacing the runtime creates a binary that links with a newer-minimum dylib and is not supported.
- SwiftPM release binaries must retain `@executable_path/../Frameworks` in `LC_RPATH`. For certificate-less local DMGs, use `scripts/sign-local-app.sh`; it omits Hardened Runtime and applies a stable explicit designated requirement so TCC permissions survive rebuilds.
- Menu bar icons must load through `Bundle.image(forResource:)` with an SF Symbol fallback. SwiftPM excludes the asset catalog, and manual app bundles contain raw menu icon PNGs rather than `Assets.car`.
...
- Temporary paste insertion must use `.currentHostOnly` plus `org.nspasteboard.TransientType`, `org.nspasteboard.AutoGeneratedType`, and `org.nspasteboard.source` on the same pasteboard item. This keeps paste as the compatible Web/Electron path while asking compliant clipboard-history tools not to retain dictated text. Do not use `ConcealedType` by default because it means confidential content, not merely generated content.
...
- If full Xcode is unavailable, `scripts/package-dmg.sh` uses `scripts/build-swiftpm-app.sh` to assemble and sign a Release app from SwiftPM, the bundled sherpa/ONNX runtime, icons, and checksum-verified local models.
- Successful local DMG packaging runs `scripts/fresh-install-local.sh` by default. It validates the staged app before changing the machine, then removes the active old app/data from their live locations, resets TCC for `com.end.DesktopVoiceInput`, installs `/Applications/GuGuTalk.app`, and launches it. Removed files are retained in a timestamped Trash directory. Use `GUGUTALK_SKIP_POST_PACKAGE_INSTALL=1` only for artifact-only CI or an explicitly requested no-install package.
- The fresh-install hook must register the staged app with Launch Services before calling `tccutil reset`; otherwise a previously uninstalled or already-unregistered bundle can fail permission reset with OSStatus `-10814`.
